...
def setup_model(generator_data, bus_data, branch_data, task1=True, eec=False, output_pmaxes=True):
    ...
    for g in generators:
        ...
        gamma[g.generator] = model.addVar(name=f'Γ_new{g.generator}', lb=0, ub=ub)  # add gamma to model wrt constraint (5)
        if eec:
    ...
    for i in buses:
        ...
        for generator in G_i:
            if generator.fuel == 'wind':
                ...
                gammas.append(bib[generator.generator] * gamma[generator.generator] + bia[generator.generator] * new_gamma[generator.generator])
